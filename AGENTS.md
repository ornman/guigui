...
**偏好:执行计划文档落盘时,回复必须附上新对话启动话术**

1. 每当把修复计划/执行计划写入仓库(如 `docs/plans/*.md`),最终回复必须包含两件套:
   - 计划文档路径(markdown 链接,可点击);
   - 一句用户可直接复制的新对话启动话术,指明文档路径与起始项(如「执行 docs/plans/xxx.md 的 P0」)。
2. 计划文档内应自带「执行约定」节(测试基准/契约变更规则/不顺手修等),让新对话零上下文可执行。

**Why:** 2026-09-07 QA 修复计划(qa-fix-plan-2026-09-07.md)用户要开新对话执行;只落盘不给话术,用户得自己拼上下文,交接有摩擦。

**How to apply:** 写完 plan 文件 + git commit 后,最终回复固定带「路径 + 启动话术」;计划分段(P0/P1/…)时话术默认指到最优先段。
