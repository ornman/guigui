...
**偏好:执行计划文档落盘时,回复必须附上启动话术;纯执行项 agent 可自动接力**

1. 每当把修复计划/执行计划写入仓库(如 `docs/plans/*.md`),最终回复必须包含两件套:
   - 计划文档路径(markdown 链接,可点击);
   - 一句启动话术,指明文档路径与起始项(如「执行 docs/plans/xxx.md 的 P0」)——用户复制开新对话、或 agent 喂给 subagent 接力,同一句都成立。
2. 计划文档内应自带「执行约定」节(测试基准/契约变更规则/不顺手修等),让执行上下文(新对话或 subagent)零上下文可执行。
3. **执行接力分工(2026-09-11 与用户拍板)**:计划余下项分两类——
   - **纯执行项**(无产品决策:代码/测试/mock/画廊/矩阵/走查对账),当前会话可 spawn subagent 自动接力(启动话术即 prompt),不必等用户手动开新对话;
   - **拍板项**(文案定稿/契约 bump/方案取舍),无论哪种执行方式都必须暂停,收拢成清单呈报用户裁决,不得由 subagent 代拍;
   用户明说「继续/接力/跑完」时按本条走;没说则默认两件套等手动。

**Why:** 2026-09-07 起「手动复制开新对话」是路径依赖(上下文健康 + 用户逐项把关),非能力限制——subagent 本就是零上下文执行上下文;2026-09-11 用户确认自动接力可行,但拍板点不能被链式执行冲掉。

**How to apply:** 写完 plan 文件 + git commit 后,最终回复固定带「路径 + 启动话术」;收尾批次可自动接力跑到计划内可执行项全部闭环,拍板项一次呈报;计划分段(P0/P1/…)时话术默认指到最优先段。

## 工程化文档体系(2026-09-11 立)

**偏好:开工先读总纲,流程/模板/坑按治理文档走**

1. **项目总纲 = `docs/SPEC.md`**:产品定位、架构基线、质量基线(测试/契约/验收)、决策基线(已拍板不可随手推翻清单)、文档地图。任何会话接手项目先读它。
2. 治理文档(`docs/gov/`):`sdlc.md`(单人+AI 协作六阶段,每阶段门禁)/ `document-management.md`(目录宪法、常青 vs 快照、单一正本)/ `prompt-templates.md`(三类提示词模板:新功能 T1 / 排查 T2 / 零上下文接力 T3)/ `ops.md`(构建/发版/坑表)。
3. 规划:`docs/plans/roadmap-2026-09.md`(P0 拍板项 / P1 验收欠账 / P2 工程化)。
4. 新文档按目录宪法落位;**常青文档改即生效,快照文档(计划/审计/ADR)落盘不改写**;转抄事实一律写「见 <正本>」不复制数字。

**Why:** 单人+全栈+agent 方向(2026-09-10 拍板)需要流程落盘:拍板不可被链式执行冲掉、教训不可每次重踩、任意新会话零上下文可接手。

**How to apply:** 接需求先对 `docs/gov/sdlc.md` 定阶段;派 subagent 用 `prompt-templates.md` T3 模板;踩新坑先回填 `ops.md` 坑表;翻车复盘后回填模板。
