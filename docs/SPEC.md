...
> **基线日期**:2026-09-12 · 后端测试 333 绿 · 契约 v1.7.0 · 杀软整改批 ADR-0001~0004 已落地。
...
| 项 | 基线 | 怎么验 |
|---|---|---|
| 后端测试 | **333 测全绿**(回归底线,变更只增不减) | 仓库根:`.venv-guigui/Scripts/python.exe -m pytest guigui/tests -q` |
| 前后端契约 | docs/tech/guigui-bridge-api-v1.md **v1.7.0** | 改接口必须 bump + 登记变更记录 + 双端适配,禁静默改 |
...
| 通知语义 = 非假期连不上也发(纯诊断)/假期(自动 3 天或手动,任一)全静默/成功默认开 | 2026-09-12 | PRD §4.5;契约 §3.1 |
| 任务停试退避 = 连败 3 次停试,rebuild 唯一恢复入口 | 2026-09-12 | ADR-0006 |
...
- **待拍板**:ADR-0007 库链设计草案(批C 前过目)、H 表拟稿场景化定稿(见 docs/plans/roadmap-2026-09.md 挂账清单)。
- **已知风险**:全链未签名(审计 S2,最大误报放大器;ADR-0005 路线 d 实施链待用户 GitHub 建仓);真机验收欠账(锚点窗口期/bind 解绑/返校日);Dr.COM 明文 HTTP 为协议固有(审计 R8,不可整改)。
