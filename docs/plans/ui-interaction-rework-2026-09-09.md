...
### P0-8 首装+waiting 专属等门(09-09 三轮入册,D3;用户拍板直接进 plan)
- 病灶:未配置+waiting(晚上装、网卡慢、网线没插)→ `firstRun` 把 waiting 压成 down → `goSetup` → **「这条路今天不通。」排查页**——用户和产品的第一见面是死亡宣判,还被建议换 WiFi。已配置用户有等门页,**未配置用户没有**:同一状态两种人格。
- 修法:`firstRun` 在 `!configured && s==='waiting'` 时不落 v-guide,复用 `bootWait` 的壳换首装文案(拟稿「网络还没就绪,等它一下…连上就带你登录」);收到 `not_logged_in`/网就绪 → `goSetup()` 落表单;`unreachable`(真断网)才落排查页。
- 验收:模拟首装 waiting(mock 需补首装 waiting 场景或用 `_setNet` 驱动)→ 等门样式而非「不通」;就绪后落 v-login;真 unreachable 仍走排查页。
...
### P1-8 等门页 GUI 侧自轮询(09-09 三轮入册,D7;用户拍板直接进 plan)
- 病灶(代码查证):GUI 常开时**自己从不探网**——一切 net:state 事件 = FileWatcher(2s)翻译外部 `--ensure` 进程落盘;「每 30 秒看一次」的执行者是 AtLogon 拉起的 ensure 进程。**任务被拦的用户(task_ok=false,恰是拦截页那批人)停在等门页会永远等**,UI 不告知。
- 修法:等门态(日常 bootWait 与 P0-8 新增的首装等门)挂 JS 侧 30s 轮询 `GG.api.probe()`,结果按现有 net:state 路由走——事件与轮询双保险,任务不在岗也能开门。轮询句柄随视图离开清理。
- 验收:blocked 场景(taskOk=false)停在等门页 → 30s 内自转(测试可临时调短间隔);开门链照旧;离开等门页无泄漏定时器。
