...
### P1-11 通知深链(2026-09-10 新增;契约 §6 backlog 提级,依赖 J 章通知策略)
- 通知点击 → 打开 GUI 并直达对应视图:拦截页 / 登录页 / 大头状态页(P1-10)/ 问题反馈。
- 契约侧:新增启动路由参数(`open_route`),后端启动时透传给前端路由;契约版本随之 bump(执行时与后端计划对齐)。
- 验收:mock 注入通知路由 → 打开落点正确;无路由参数时行为与现状完全一致。

> **2026-09-10 白板同步说明**:飞书业务流程正本(docs/prd/flows/guigui-user-flow.dsl.json)已按同日拍板完成 g3 日常链四带重构(状态页=问题中枢/日常页两态/后端返回五态/L1L2 探测后台常驻/检验自适应服务/登录页合并/复用图例),本计划 P1-8/P1-10 的口径以白板 2026-09-10 版为准;执行时若与上文冲突,以白板+J/K 章为准。
...
| 事件 | 通知 | 深链落点 |
|---|---|---|
| 例行任务成功(vok) | ✅ 默认开·可关 | 无(纯告知) |
| 任务最终失败·凭据类(vfail:密码/学号运营商/绑定/别设备) | ✅ 带原因+下一步 | 登录页/大头状态页 |
...
- **通知点击 = 深链**:见 P1-11。
- 验收:mock 各 🔔 场景触发一次,冷却窗口内二连失败只见到一条;成功通知开关关闭后 vok 静默。

## K. 模块复用表(2026-09-10;执行硬约束——白板同标记 = 实现一份,禁止复制粘贴)

| 标记 | 模块 | 白板出现位置 | 约束 |
|---|---|---|---|
| 🅛 | 后端登录五态机 | g2.后端判断 / g3.后端返回 | 同一后端服务;首装表单/快登/检验共用 |
