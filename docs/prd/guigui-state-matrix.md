...
| v-ok | 已连已登 / 学号已识别 / 验证中 busy / 注销告知 / 密码空警告 / 被拒(wrong_password / bound) / 节流 note;停留断网 → 直跳 v-guide(表单数据保留) |
| v-login | 未登录落地 / 被拒五态(wrong_password / wrong_account / bound / limit_users / throttled) / 验证中(注明他人学号) / 场景4 注销后失败两变体(网先断着 / 已接回) / 验证器结论预填(与被拒同组件同位置) / v-guide ③ 路径(按钮=「保存配置」);换学号确认框已删(密码迁移静默,验证不过当场自纠);停留断网 → 直跳 v-guide |
...
| v-guide | 留在排查页(它正是落点) | 自动回主页 ok | 自动跳 v-login |
| v-main | down 态重渲(「去排查」按钮;主页有自己的四态,不自动跳) | ok 重渲 | out 重渲 |
| v-login / v-ok | **直跳 v-guide**(表单数据保留,回来还在;原先只刷状态行,2026-09-08 撤) | 状态行实时刷新 + mainState 记对(返回主页不混搭) | 同左 |
...
| 3 | 不可达不登录 | →v-guide(挑网→连上→自动跳 v-login);任何视图停留断网也直跳此页 | S3 / S5 |
...
- 状态画廊:`guigui/app/static/dev/state-gallery.html`(仅 dev,打包排除 `static/dev/`;10 视图 × 全部状态帧 + 十四故事,须 http 打开);新增状态必须同步画廊帧,否则美术看不到。
