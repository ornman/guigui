...
## 五、联调后新增差异与修复(2026-08-31 晚)

| # | 事项 | 性质 |
|---|---|---|
| 9 | 窗口圆角改为 `SetWindowRgn` 裁剪实现(WinForms+WebView2 无真透明,四角露窗体白底;用户实机发现) | 平台限制;**用户拍板**:22px 精确圆角优先,接受卡片外投影被裁 |
| 10 | 新增 v-feedback 视图 + 设置页「遇见问题?」入口行(契约 1.1.0 `feedback()`) | **用户拍板**(后端对话登记):反馈动作=复制打码诊断;复用原型元件(back/h1/lede/.log 深面板/.btn),样式零新增 |
| 11 | 主页第三行标题改动态 `when`(id=main-last-t),缺省「昨晚」 | 联调实测修复:今早记录与静态「昨晚」打架 |
| 12 | `lastText` 对 `tries===0` 映射「(时间) 已经在线 ✓」 | 联调实测修复:「第 0 次」不可读 |

> 窗口圆角配方与实测证据见契约「集成待办」;devshell.py 为参考实现。
