...
- pywebview 5,WebView2 后端:560×640、`frameless=True`、`resizable=False`、无边框 + `SetWindowRgn` 圆角裁剪,半径 **8 CSS px × DPI**(2026-08-31 二次拍板,对齐 Win11 系统圆角;曾短暂试过原生窗口当日内回退)、`background_color='#2b2740'` 兜弧线缝隙与首帧闪白。配方详见契约 §6。
...
| AC | 验法 | 结果 |
|---|---|---|
| AC-01 | 前夜配置 07:00;07:00 开机,AtLogon ensure 登录;`ping baidu.com` 30s 内通 | **2026-08-31 真机过(部分)**:任务注册成功(每天 06:30 起 5 分钟×25 窗口,动作 = 打包 exe --ensure),`schtasks /run` 手动触发 → 退出码 0 → 三条真实日志落盘(网络可达/已登录·打码学号/下班啦)。**部分**:LogonTrigger(开机补登录)被火绒按内容拦(XML 含登录触发的导入一律"拒绝访问",与父进程无关),当前任务无开机触发;火绒行为信任修好后删任务重建即可补全 |
...
| AC-05 | 关窗后 `tasklist | findstr guigui` 空;ensure 运行时 RSS <50MB | 关窗即退已实测(rc=0);开窗 WS ≈620MB 全为 WebView2 引擎(用户已接受,见记忆);ensure 短进程 RSS 待采样 |
| AC-06 | 全天不打开 GUI,schtasks 触发日志显示独立短进程完成登录 | 机制已随 AC-01 验证(手动触发);"全天自动"留待明晨 06:30 自然窗口观察 |
...
7. **窗口透明/圆角(已结)**:真透明 WebView2 确认做不到,落 SetWindowRgn 裁剪配方(半径 8px,对齐 Win11 系统圆角;原生窗口方案 2026-08-31 当日试用后回退)。
