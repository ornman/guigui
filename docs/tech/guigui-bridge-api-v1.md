...
- **wake_login 默认值分歧(后端记)**:契约 §2.6 示例与 mock 均为 `true`,PRD §5 表为默认**关**。后端按 PRD 实现 `false`(技术方案 §14.5)。需拍板:改 mock 为 false(对齐 PRD)或改 PRD 默认为开;未拍板前联调首屏该开关态会不一致。
- **deep link 注入形状(后端记)**:壳在 loaded 后注入 `window.__guigui_launch = 'main|creds|settings'`(guigui/app/gui.py),app.js 尚未消费。通知点击路由(guigui://main / creds)依赖它,建议下次契约修订时收编进 §4 启动时序。
- **学号打码示例说明(后端记)**:打码规则=前4…后4,对真实学号 2025000000001 得 `2025…0001`;PRD/契约示例里的 `2025…7209` 是手打示意串、非规则推得。前端如做正则校验请以规则为准。
